...
cp1(:,:,:,:), cp2(:,:,:,:), cv1(:,:,:,:), cv2(:,:,:,:), cx1(:,:,:,:), cx2(:,:,:,:)
    ...
    if (flag==0) then
		...
		! eqs. (3) and (4)
		f0(1) = -p1 + bet*p1f + kap1*u1 + kap1*((rho+eta+chi*varpi*rho)*y1 - chi*varpi*rho*y2)
	    f0(2) = -p2 + bet*p2f + kap2*u2 + kap2*((rho+eta+chi*varpi*rho)*y2 - chi*varpi*rho*y1)
	    ! f0(1) = -p1 + bet*p1f + kap1*u1 + kap1*((rho+eta+0.5d0*(1.0d0-rho*iota)/iota)*y1 - 0.5d0*(1.0d0-rho*iota)/iota*y2)
	    ! f0(2) = -p2 + bet*p2f + kap2*u2 + kap2*((rho+eta+0.5d0*(1.0d0-rho*iota)/iota)*y2 - 0.5d0*(1.0d0-rho*iota)/iota*y1)
    ...
    elseif (flag==1) then
	    ...
	    ! f0(2) = -p2 + bet*p2f + kap2*u2 + kap2*((rho+eta+0.5d0*(1.0d0-rho*iota)/iota)*y2 - 0.5d0*(1.0d0-rho*iota)/iota*y1)
		! eq. (1)
		f0(3) = -w1 -.5d0*((eta+rho)*(y1 - 1.0d0/(eta+rho)*u1)**2 &
    ...
    elseif (flag==2) then
	    ...
	    ! f0(2) = -p2 + bet*p2f + kap2*u2 + kap2*((rho+eta+0.5d0*(1.0d0-rho*iota)/iota)*y2 - 0.5d0*(1.0d0-rho*iota)/iota*y1)
		! eq. (2)
		f0(3) = -w2 -.5d0*((eta+rho)*(y2 - 1.0d0/(eta+rho)*u2)**2 &
...
subroutine calcp(p1,p2,y1,y2,z,x,u1p,u2p,u1,u2,xp,y1p,y2p,w11,w12,cx1,cx2)
	...
	eta1p = (xp-0.5d0)*u1p - bet/2.0d0*xi1
    eta2p = (xp-0.5d0)*u2p + bet/2.0d0*xi2
	zet1p =  ((eta+rho+chi*varpi*rho)*eta1p-chi*varpi*rho*eta2p)/(.5d0*(eta+rho+2*chi*varpi*rho)*(eta+rho))
	zet2p = -((eta+rho+chi*varpi*rho)*eta2p-chi*varpi*rho*eta1p)/(.5d0*(eta+rho+2*chi*varpi*rho)*(eta+rho))
    ! zet1p =  ((eta+rho+0.5d0*(1.0d0-rho*iota)/iota)*eta1p-0.5d0*(1.0d0-rho*iota)/iota*eta2p)/((eta+rho+(1.0d0-rho*iota)/iota)*(eta+rho))
	! zet2p = -((eta+rho+0.5d0*(1.0d0-rho*iota)/iota)*eta2p-0.5d0*(1.0d0-rho*iota)/iota*eta1p)/((eta+rho+(1.0d0-rho*iota)/iota)*(eta+rho))
	...
	eta1 = (2.0d0*x-1.0d0)*u1 - bet*xi1f
    eta2 = (2.0d0*x-1.0d0)*u2 + bet*xi2f
	zet1 =  ((eta+rho+chi*varpi*rho)*eta1-chi*varpi*rho*eta2)/((eta+rho+2*chi*varpi*rho)*(eta+rho))
    zet2 = -((eta+rho+chi*varpi*rho)*eta2-chi*varpi*rho*eta1)/((eta+rho+2*chi*varpi*rho)*(eta+rho))
    ! zet1 =  ((eta+rho+0.5d0*(1.0d0-rho*iota)/iota)*eta1-0.5d0*(1.0d0-rho*iota)/iota*eta2)/((eta+rho+(1.0d0-rho*iota)/iota)*(eta+rho))
    ! zet2 = -((eta+rho+0.5d0*(1.0d0-rho*iota)/iota)*eta2-0.5d0*(1.0d0-rho*iota)/iota*eta1)/((eta+rho+(1.0d0-rho*iota)/iota)*(eta+rho))
