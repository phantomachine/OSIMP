...
cp1(:,:,:,:), cp2(:,:,:,:), cv1(:,:,:,:), cv2(:,:,:,:), cx1(:,:,:,:), cx2(:,:,:,:)
    ...
    if (flag==0) then
		...
		! eqs. (3) and (4)
		f0(1) = -p1 + bet*p1f + kap1*u1 + kap1*(1.0d0+eta)*y1
	    f0(2) = -p2 + bet*p2f + kap2*u2 + kap2*(1.0d0+eta)*y2
    ...
    elseif (flag==1) then
	    ...
	    f0(2) = -p2 + bet*p2f + kap2*u2 + kap2*(1.0d0+eta)*y2
		! eq. (1)
		f0(3) = -w1 -0.5d0*(2.0d0*(1.0d0+eta)*y1**2 - (2.0d0-neu)*(1.0d0+eta)*(y1 + 1.0d0/(eta+1.0d0)*u1)**2 + 2.0d0*(1.0d0/(eta+1.0d0)*u1)**2 &
		+ (2.0d0-neu)*(1.0d0+eta)*(y2 + 1.0d0/(eta+1.0d0)*u2)**2 &
		+ sig*(2.0d0-neu)/kap1*p1**2 + sig*neu/kap1*p2**2) + bet*v1f
    ...
    elseif (flag==2) then
	    ...
	    f0(2) = -p2 + bet*p2f + kap2*u2 + kap2*(1.0d0+eta)*y2
		! eq. (2)
		f0(3) = -w2 -0.5d0*(2.0d0*(1.0d0+eta)*y2**2 - (2.0d0-neu)*(1.0d0+eta)*(y2 + 1.0d0/(eta+1.0d0)*u2)**2 + 2.0d0*(1.0d0/(eta+1.0d0)*u2)**2 &
		+ (2.0d0-neu)*(1.0d0+eta)*(y1 + 1.0d0/(eta+1.0d0)*u1)**2 &
		+ sig*(2.0d0-neu)/kap2*p1**2 + sig*neu/kap2*p2**2) + bet*v2f
...
subroutine calcp(p1,p2,y1,y2,z,x,u1p,u2p,u1,u2,xp,y1p,y2p,w11,w12,cx1,cx2)
	...
	eta1p = 0.5d0*(2.0d0-neu)*(2.0d0*xp-1.0d0)*u1p - bet/2.0d0*Xi1
    eta2p = 0.5d0*(2.0d0-neu)*(2.0d0*xp-1.0d0)*u2p + bet/2.0d0*Xi2
    zet1p =  eta1p/(.5d0*(neu*xp+(2.0d0-neu)*(1.0d0-xp))*(eta+1.0d0))
	zet2p = -eta2p/(.5d0*((2.0d0-neu)*xp+neu*(1.0d0-xp))*(eta+1.0d0))
	...
	eta1 = (2.0d0-neu)*(2.0d0*x-1.0d0)*u1 - bet*xi1f
    eta2 = (2.0d0-neu)*(2.0d0*x-1.0d0)*u2 + bet*xi2f
    zet1 =  eta1/((neu*x+(2.0d0-neu)*(1.0d0-x))*(eta+1.0d0))
    zet2 = -eta2/(((2.0d0-neu)*x+neu*(1.0d0-x))*(eta+1.0d0))

	p1 = -1.0d0/sig*(y1-zet1-z*(neu*xp+(2.0d0-neu)*(1.0d0-xp))/(neu*x+(2.0d0-neu)*(1.0d0-x))*(y1p-zet1p))
    p2 = -1.0d0/sig*(y2-zet2-z*((2.0d0-neu)*xp+neu*(1.0d0-xp))/((2.0d0-neu)*x+neu*(1.0d0-x))*(y2p-zet2p))
