...
eval(['load ../ncp/mat/pfnc_', filename, '.mat'])    
...
for i = 1:N
    ...
    for t = 1:T
        ...
        y1vec(t+1) = intf2(knotsy,knotsy,reshape(y1mat(ku,ju,:,:),[ny ny]),y1vec(t),y2vec(t));
        y2vec(t+1) = intf2(knotsy,knotsy,reshape(y2mat(ku,ju,:,:),[ny ny]),y1vec(t),y2vec(t));
        p1vec(t+1) = intf2(knotsy,knotsy,reshape(p1mat(ku,ju,:,:),[ny ny]),y1vec(t),y2vec(t));
        p2vec(t+1) = intf2(knotsy,knotsy,reshape(p2mat(ku,ju,:,:),[ny ny]),y1vec(t),y2vec(t));    
        v1vec(t+1) = intf2(knotsy,knotsy,reshape(v1mat(ku,ju,:,:),[ny ny]),y1vec(t),y2vec(t));
        v2vec(t+1) = intf2(knotsy,knotsy,reshape(v2mat(ku,ju,:,:),[ny ny]),y1vec(t),y2vec(t));    
        ...
        for lu = 1:nu
            ...
            p1f = p1f + Pu(ju,lu)*intf2(knotsy,knotsy,reshape(p1mat(ju,lu,:,:),[ny ny]),y1,y2);
            p2f = p2f + Pu(ju,lu)*intf2(knotsy,knotsy,reshape(p2mat(ju,lu,:,:),[ny ny]),y1,y2);
            v1f = v1f + Pu(ju,lu)*intf2(knotsy,knotsy,reshape(v1mat(ju,lu,:,:),[ny ny]),y1,y2);
            v2f = v2f + Pu(ju,lu)*intf2(knotsy,knotsy,reshape(v2mat(ju,lu,:,:),[ny ny]),y1,y2);
...
ev1(3) = v1mat(5,5,midy,midy);
ev1(4) = v2mat(5,5,midy,midy);
