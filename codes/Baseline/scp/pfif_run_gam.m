...
for j = 1:size(gvec,2)
    ...
    for i = 1:size(rvec,2)
        ...
        rho = rvec(i);
        filename = ['g', num2str(gvec(j),'%1.3f'), '_ny', num2str(n1,'%1d'), 'nx', num2str(nx,'%1d'), 'ymax', num2str(ymax,'%1.1f'), '_rho', num2str(rho,'%1.1f')];
        disp(filename);
        calcscp;
        eval(['save ./mat/pfsc_', filename, '.mat bet eta rho sig kap1 kap2 gam Gu Pu knots1 knots2 knotsx x1mat x2mat y1mat y2mat p1mat p2mat v1mat v2mat zmat xmat w1mat w2mat;']);    
        plotif;
