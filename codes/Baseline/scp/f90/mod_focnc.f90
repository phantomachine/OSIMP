...
function focnc(xx0,u1p,u2p,u1,u2,y1p,y2p,cpn1,cpn2) result(f0)
    ...
    real(8), intent(in) :: u1p, u2p, u1, u2, y1p, y2p, cpn1(:,:,:), cpn2(:,:,:)
    ...
    ! trade-off eqs.
    xi1  = (1.0d0-gam)/gam*((eta+rho+gam*(1.0d0-rho))*u1 -gam*(1.0d0-rho)*u2 )/((1.0d0+eta)*(rho+eta))
    xi1p = (1.0d0-gam)/gam*((eta+rho+gam*(1.0d0-rho))*u1p-gam*(1.0d0-rho)*u2p)/((1.0d0+eta)*(rho+eta))
    xi2  = gam/(1.0d0-gam)*((eta+rho+(1.0d0-gam)*(1.0d0-rho))*u2 -(1.0d0-gam)*(1.0d0-rho)*u1 )/((1.0d0+eta)*(rho+eta))
    xi2p = gam/(1.0d0-gam)*((eta+rho+(1.0d0-gam)*(1.0d0-rho))*u2p-(1.0d0-gam)*(1.0d0-rho)*u1p)/((1.0d0+eta)*(rho+eta))
...
subroutine csolve(x,rc,crit,itmax,u1p,u2p,u1,u2,y1p,y2p,cpn1,cpn2)
...
!    f0 = func(x,varargin)
!    f0 = func(x)
    f0 = focnc(x,u1p,u2p,u1,u2,y1p,y2p,cpn1,cpn2)
    af0 = sum(abs(f0))
    af00 = af0
    itct = 0
    rc = 0
    ...
    !    if ~analyticg
        ...
        do iv = 1,nv
            ...
            x0 = x
            x0(iv) = x(iv) + delta ! tvec
            !f1 = func(x0,varargin)
!            f1 = func(x0)
            f1 = focnc(x0,u1p,u2p,u1,u2,y1p,y2p,cpn1,cpn2)
            grad(:,iv) = (f1-f0)/delta
...
!        call dgetri(nv,grad,nv,IPIV,WORK,nv,INFO)
    ...
    !    end if
        ...
        do while (subDone == 0)
            ...
            dx = lambda*dx0
!            f = func(x+dx,varargin)
!            f = func(x+dx)
            f = focnc(x+dx,u1p,u2p,u1,u2,y1p,y2p,cpn1,cpn2)
            af = sum(abs(f))
